...
class RiskService(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.case_table_name = CONFIG.get("database", "dead_birds_table_projected")
        self.tmp_daily_case_table = CONFIG.get("database", "tmp_daily_case_table")
        self.tmp_cluster_per_point_selection_table = CONFIG.get("database", "tmp_cluster_per_point_selection_table")        
            ...
            if daily_case_count >= case_threshold:
                ...
                for point in gridpoints:
                    ...
                    if vector_count >= case_threshold:
                        ...
                        self.insert_cases_in_cluster_table(dycast_parameters, point, cur, conn)
                        results = self.cst_cs_ct_wrapper(dycast_parameters, cur, conn)
                        close_pairs = results[0][0]
                        close_space = results[1][0] - close_pairs
                        close_time = results[2][0] - close_pairs
                        result2 = self.nmcm_wrapper(vector_count, close_pairs, close_space, close_time, cur, conn)
                        self.insert_result(day, point.x, point.y, vector_count, close_pairs, close_time, close_space, result2[0][0], cur, conn)
    ...
    def setup_tmp_daily_case_table_for_date(self, dycast_parameters, riskdate, cur, conn):
        ...
        querystring = "TRUNCATE " + self.tmp_daily_case_table + "; INSERT INTO " + self.tmp_daily_case_table + " SELECT * from " + self.case_table_name + " where report_date >= %s and report_date <= %s"
        try:
    ...
    def get_vector_count_for_point(self, dycast_parameters, point, cur, conn):
        querystring = "SELECT count(*) from \"" + self.tmp_daily_case_table + "\" a where st_distance(a.location,ST_GeomFromText('POINT(%s %s)',%s)) < %s"
        try:
            cur.execute(querystring, (point.x, point.y, self.system_coordinate_system, dycast_parameters.spatial_domain))
        except Exception:
            conn.rollback()
    ...
    def insert_cases_in_cluster_table(self, dycast_parameters, point, cur, conn):
        querystring = "TRUNCATE " + self.tmp_cluster_per_point_selection_table + "; INSERT INTO " + self.tmp_cluster_per_point_selection_table + " SELECT * from " + self.tmp_daily_case_table + " a where st_distance(a.location,ST_GeomFromText('POINT(%s %s)',%s)) < %s"
        try:
            cur.execute(querystring, (point.x, point.y, self.system_coordinate_system, dycast_parameters.spatial_domain))
        except Exception:
            conn.rollback()
