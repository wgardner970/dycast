#$Id: dycast.py,v 1.13 2008/04/01 00:16:49 alan Exp alan $
# DYCAST functions
...
def create_daily_risk_table(riskdate):
    logging.info("daily_risk: creating risk table2")
    ...
    try:
        ...
        logging.info("daily_risk: creating risk table exception")
        conn.rollback()
        logging.info("daily_risk: creating risk table exception2")
        # TODO: save old version of risk instead of overwriting
        try:   
            cur.execute("DROP TABLE \"" + tablename + "\"")
            cur.execute(querystring)
            logging.info("daily_risk: creating risk table exception3")
        except:
            logging.info("daily_risk: creating risk table exception4")
            conn.rollback()
            logging.error("couldn't create risk table: %s", tablename)
            sys.exit()
    logging.info("daily_risk: creating risk table exception5")
    conn.commit()
    return tablename
...
def daily_risk(riskdate, startpoly=None, endpoly=None):
    logging.info("daily_risk: getting ids")
    rows = get_ids(startpoly, endpoly)

    logging.info("daily_risk: creating risk table")
    risk_tab = create_daily_risk_table(riskdate)
    logging.info("daily_risk: creating bird table")
    ...
    for row in rows:
        ...
        if num_birds >= threshold:
            #print "tile id:", tile_id, "found", num_birds, "birds (above threshold)"
    ...
    logging.info("daily_risk for %s done %s tiles, time elapsed: %s", riskdate, inc, time.time() - st)
